    ...
    configureServer(server) {
      const handler: Connect.NextHandleFunction = async (req, res, next) => {
        ...
        if (url.pathname === "/api/proxy/hls") {
          ...
          try {
            const isTwitch = target.hostname.includes("ttvnw.net") || target.hostname.includes("twitch.tv");
            const upstreamHeaders = {
              "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36",
              "Referer": isTwitch ? "https://www.twitch.tv/" : target.origin + "/",
              "Origin": isTwitch ? "https://www.twitch.tv" : target.origin,
              "Accept": "*/*",
              "Accept-Language": "en-US,en;q=0.9",
              "Accept-Encoding": "gzip, deflate, br",
              ...(isTwitch ? { "Sec-Fetch-Dest": "empty", "Sec-Fetch-Mode": "cors", "Sec-Fetch-Site": "cross-site" } : {}),
            };
