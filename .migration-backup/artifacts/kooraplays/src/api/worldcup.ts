...
const STADIUM_UTC_OFFSET: Record<string, string> = {
  "1":  "-05:00",
  "2":  "-07:00",
  "3":  "-04:00",
  "4":  "-07:00",
...
function parseLocalDate(localDate: string, stadiumId: string): string {
  try {
    ...
    const offset = STADIUM_UTC_OFFSET[stadiumId] ?? "-05:00";
    return `${year}-${month.padStart(2, "0")}-${day.padStart(2, "0")}T${timePart}:00${offset}`;
  } catch {
    return new Date().toISOString();
  }
}

function getStatus(game: RawGame): "LIVE" | "UPCOMING" | "FINISHED" {
