    ...
    <div className="flex flex-col min-h-screen">
      <section className="relative overflow-hidden bg-card border-b border-border/40 py-12 md:py-20 px-4 md:px-6">
        ...
        <div className="container mx-auto relative z-10 flex flex-col items-start gap-4 md:gap-6 max-w-5xl">
          <Badge variant="outline" className="px-3 py-1 text-xs md:text-sm border-primary/30 text-primary bg-primary/5">
            <Trophy className="h-3 w-3 md:h-4 md:w-4 me-2" />
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mt-2 md:mt-4 w-full sm:w-auto">
            <Link href="/live" className="w-full sm:w-auto">
              <Button size="lg" className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-primary-foreground text-base md:text-lg px-6 md:px-8 h-12 md:h-14 rounded-full font-bold">
                <PlayCircle className="me-2 h-5 w-5" />
                {t.startWatching}
              </Button>
            </Link>
            <Link href="/schedule" className="w-full sm:w-auto">
              <Button variant="outline" size="lg" className="w-full sm:w-auto h-12 md:h-14 px-6 md:px-8 rounded-full font-semibold">
                {t.fullSchedule}
              </Button>
            </Link>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {displayMatches.map((match, i) => (
                ...
                >
                  {match.status === "LIVE" ? (
                    <Link href="/live" className="block cursor-pointer">
            ...
            <div className="text-center py-20 text-muted-foreground bg-card rounded-xl border border-border">
              ...
              <Link href="/schedule">
                <Button variant="outline" className="mt-4" data-testid="link-schedule">{t.viewSchedule}</Button>
              </Link>
            </div>
          )}
        </section>
