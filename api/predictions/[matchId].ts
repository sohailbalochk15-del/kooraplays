...
async function getCounts(matchId: string): Promise<{ home: number; draw: number; away: number; total: number }> {
  ...
  try {
    // The match_predictions table stores ONE row per (match_id, outcome) with an
    // aggregate count column — not one row per vote. Select outcome + count directly.
    ...
    for (const row of rows) {
      ...
      if (row.outcome === "home")      result.home = n;
      else if (row.outcome === "draw") result.draw = n;
      else if (row.outcome === "away") result.away = n;
    }
